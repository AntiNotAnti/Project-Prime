...
public sealed class VisualLightPolicyTests
{
    ...
    public void SelectionIsStableAcrossInputReorderingAndRespectsBound()
    {
        VisualLightCandidate[] candidates = Enumerable.Range(0,
            VisualLightSelection.MaximumSupportedLights + 12)
        ...
        Assert.Equal(selectedPriorities.OrderByDescending(value => value),
            selectedPriorities);
        Assert.All(selectedPriorities.Take(8), priority => Assert.Equal(4,
            priority));
    }
