...
public sealed class EnhancedMaterialTests : IDisposable
{
    ...
    [Fact]
    public void CookedTextureRoundTripsAndLoadsThroughTheNormalResolver()
    {
        byte[] pixels =
        {
            255, 0, 0, 255,
        ...
        byte[] cooked = EnhancedTextureCookedFormat.Encode(2, 1, pixels);
        Assert.True(EnhancedTextureCookedFormat.TryDecode(cooked,
            out int width, out int height, out byte[] decoded));
        Assert.Equal(2, width);
        Assert.Equal(1, height);
        Assert.Equal(pixels, decoded);

        Directory.CreateDirectory(Path.Combine(_root, "textures"));
        File.WriteAllBytes(Path.Combine(_root, "textures", "armor.ppte"),
            cooked);
        WriteManifest(new
        {
            format = 1,
        ...
        EnhancementPackLoadResult loaded = EnhancementPackLoader.Load(_root);
        Assert.Empty(loaded.Issues);
        Assert.True(loaded.Resolver.TryGetReplacement(
            TextureAssetKey.ForModel("samus", 3, 0, 0),
            out EnhancedMaterialReplacement? replacement));
        Assert.True(replacement!.Albedo!.IsCooked);
        Assert.Equal(pixels, replacement.Albedo.Rgba8.ToArray());

        cooked[^1] ^= 0x40;
        Assert.False(EnhancedTextureCookedFormat.TryDecode(cooked,
            out _, out _, out _));
    }
