...
public sealed class VisualLightFrameIntegrationTests
{
    ...
    {
        VisualLightCandidate[] candidates = Enumerable.Range(0,
            RenderFrame.MaximumVisualLights + 16)
    ...
    public void LegacyAndProfiledAdmissionShareOneOrderIndependentBound()
    {
        ...
        int candidateCount = RenderFrame.MaximumVisualLights + 12;
        for (int i = 0; i < candidateCount; i++) Admit(forward, i);
        for (int i = candidateCount - 1; i >= 0; i--) Admit(reverse, i);
