...
public sealed class TournamentIntegrationTests
{
    ...
    public async Task ServerRecordingProducesIndexedObserverBaselineAndExplicitStorageFailure()
    {
        ...
        try
        {
            ...
            string truncatedPath = Path.Combine(directory, "truncated.fpreplay");
            ...
            using var reader = ReplayReader.Open(Path.Combine(directory, recording.ReplayId.ToString("D") + ReplayFile.Extension));
            Assert.NotNull(reader); Assert.Equal(ReplayFile.IndexedFormatVersion, reader.FormatVersion);
            var records = reader.Seek(0, out uint restored); Assert.NotNull(records); Assert.Equal((uint)0, restored);
            Assert.Contains(records, r => r.Data.Length == 2 && r.Data[0] == (byte)ReplayRecordKind.Perspective && r.Data[1] == 255);
            var clock = Assert.Single(records, r => r.Data[0] == (byte)ReplayRecordKind.Clock);
            ...
            using var linear = ReplayReader.Open(Path.Combine(directory, recording.ReplayId.ToString("D") + ReplayFile.Extension));
            Assert.Equal((byte)ReplayRecordKind.Match, linear!.ReadNext()!.Value.Data[0]);
            bool sawLaterSnapshot = false;
            while (linear.ReadNext() is { } record) if (record.Frame == 0 && record.Data[0] == (byte)ReplayRecordKind.Snapshot) sawLaterSnapshot = true;
