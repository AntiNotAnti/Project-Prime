...
{
    [Fact]
    public void DefaultProfileAppliesRequestedSecondRotationalRateDoubling()
    {
        AimAssistProfile profile = PlayerAimAssist.DefaultProfile;

        Assert.Equal(480f, profile.MaxYawRate);
        Assert.Equal(352f, profile.MaxPitchRate);
        ...
        Assert.Equal(8f, assisted.X, 5);
        Assert.Equal(-352f / 60f, assisted.Y, 5);
    }
