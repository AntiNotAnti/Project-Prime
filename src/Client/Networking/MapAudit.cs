...
namespace MphRead.Mods.Network
{
    ...
    public sealed class MapAudit : IRenderToolClient
    {
        ...
        private void StepScoreboard()
        {
            ...
            PlayerPresentation.ModForceScoreboard = show;
            if (show)
            {
                _scoreboardFrames++;
            }
        ...
        private void SampleNodeLookup(PlayerEntity player)
        {
            ...
            _nodeLookupSamples++;
            bool walkedVisible = ScenePresentation.Get(Scene).IsNodeRefVisible(walked);
            if (walkedVisible)
            {
            ...
            _nodeLookupWrong++;
            bool foundVisible = ScenePresentation.Get(Scene).IsNodeRefVisible(found);
        ...
        private void SamplePuppetNode(PlayerEntity player)
        {
            ...
            _puppetWrong++;
            if (ScenePresentation.Get(Scene).IsNodeRefVisible(walked) && !ScenePresentation.Get(Scene).IsNodeRefVisible(next))
            {
                _puppetHidden++;
            }
        }
        ...
        private int Report()
        {
            ...
            line.Append($" | effect particles {ScenePresentation.Get(Scene).ModEffectParticles}");
            if (_litSamples > 0)
            {
            ...
            // it.
            if (ScenePresentation.Get(Scene).ModEffectParticles == 0)
            {
