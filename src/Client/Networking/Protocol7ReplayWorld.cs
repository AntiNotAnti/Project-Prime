// Frozen protocol 5/6/7 world decoder. Never used for live packets.
using System;
...
namespace MphRead.Mods.Network
{
        ...
        uint Id, Vector3 Position, uint A, uint B, uint C, uint D, uint E)
    {
        ...
        public static bool TryRead(ReadOnlySpan<byte> data, out Protocol7WorldRecord record)
            => TryRead(data, legacy: false, out record);
        internal static bool TryRead(ReadOnlySpan<byte> data, bool legacy, out Protocol7WorldRecord record)
        {
            record = default;
            if (data.Length != Size) { return false; }
            record = new Protocol7WorldRecord((Protocol7WorldRecordKind)data[0], data[1], BinaryPrimitives.ReadUInt16LittleEndian(data[2..]),
            ...
            {
                Protocol7WorldRecordKind.Spawner => record.Slot == 255 && record.Flags <= 1 && record.A <= ushort.MaxValue
                    && record.B <= ushort.MaxValue && (record.C < 8 || record.C == 255),
                Protocol7WorldRecordKind.Item => record.Slot <= 21 && record.Flags == 0 && record.Id != 0,
                Protocol7WorldRecordKind.Node => (record.Slot < 8 || record.Slot == 255) && record.Flags <= 7
                ...
                Protocol7WorldRecordKind.Flag => (record.Slot < 8 || record.Slot == 255) && record.Flags <= 3
                    && record.A <= 1 && float.IsFinite(Float(record.B)) && Float(record.B) >= 0,
                Protocol7WorldRecordKind.Match => record.Id == 0 && record.Slot == 255 && record.Flags <= (legacy ? 1 : 3) && record.A is >= 3 and <= 14
                ...
                Protocol7WorldRecordKind.Lifecycle => !legacy && record.Id == 0 && record.Slot == 255 && record.Flags <= 1
                    && record.C != 0 && record.D == 0 && record.E == 0 && record.Position == Vector3.Zero
                    && (record.Flags == 0 || unchecked(record.B - record.A) <= int.MaxValue),
                Protocol7WorldRecordKind.Score => record.Id == 0 && record.Slot < 8 && record.Flags == 0,
                Protocol7WorldRecordKind.Time => record.Id == 0 && record.Slot < 8 && record.Flags == 0
    ...
    internal static class Protocol7WorldPacket
    {
        public const int HeaderSize = 20;
        public const int RecordsPerBatch = 24;
        public const int Capacity = 256;
        public const int MaxSize = HeaderSize + RecordsPerBatch * Protocol7WorldRecord.Size;
        public static int Write(Span<byte> data, uint matchId, uint revision, uint tick, ReadOnlySpan<Protocol7WorldRecord> records, int offset)
        {
            ...
            for (int i = 0; i < count; i++) { records[offset + i].Write(data.Slice(HeaderSize + i * Protocol7WorldRecord.Size, Protocol7WorldRecord.Size)); }
            return HeaderSize + count * Protocol7WorldRecord.Size;
        }
        ...
        internal static bool TryValidate(ReadOnlySpan<byte> data, uint matchId, bool legacy)
        {
            ...
            if (total is < 1 or > Capacity || offset % RecordsPerBatch != 0 || offset >= total
                || count != Math.Min(RecordsPerBatch, total - offset) || data.Length != HeaderSize + count * Protocol7WorldRecord.Size) { return false; }
            for (int i = 0; i < count; i++)
            {
                if (!Protocol7WorldRecord.TryRead(data.Slice(HeaderSize + i * Protocol7WorldRecord.Size, Protocol7WorldRecord.Size), legacy, out _)) { return false; }
            }
            return true;
        }
    }
