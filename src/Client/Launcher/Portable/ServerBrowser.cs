...
namespace MphRead.Mods.Launcher
{
    ...
    {
        public static bool Full(ServerStatus status) => status.MaxPlayers > 0 && Math.Max(0, status.Players - status.Bots) >= status.MaxPlayers;
        ...
        public static ServerBrowserEntry? QuickJoin(IEnumerable<ServerBrowserEntry> entries, GameMode? preferredMode)
            => entries.Where(e => e.Status.Compatible && e.Status.Online && e.Status.Latency >= 0
                    && e.Status.MaxPlayers > 0 && !Full(e.Status))
        ...
        public static string Details(ServerStatus s)
        {
            ...
            return $"{NetStatus.ModeName(s.Mode)} | {s.RoomKey} | {s.Players - s.Bots} humans + {s.Bots} bots / {s.MaxPlayers} | {s.Latency} ms | {time} remaining\n"
