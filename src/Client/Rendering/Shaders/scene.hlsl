...
float EvaluateSpecular(float3 normal, float3 lightDirection,
    float3 viewDirection, float smoothness)
{
    ...
    float exponent = 4.0f + 124.0f * clampedSmoothness * clampedSmoothness;
    float normalization = (exponent + 8.0f) / (8.0f * 3.14159265359f);
    return pow(saturate(dot(normal, halfVector)), exponent) * normalization;
}
    ...
    float3 specularColor, float smoothness, float visibility)
{
    ...
    float specularFactor = diffuseFactor > 0.0f
        ? EvaluateSpecular(normal, lightDirection, viewDirection, smoothness) : 0.0f;
    return (diffuseColor * lightColor * diffuseFactor
        + specularColor * lightColor * specularFactor) * visibility;
}
    ...
    bool includeSpecular, bool linearLighting)
{
    ...
    float specularFactor = includeSpecular && diffuseFactor > 0.0f
        ? EvaluateSpecular(normal, lightDirection, viewDirection, smoothness) : 0.0f;
    ...
    return radiance * (diffuseColor * diffuseFactor
