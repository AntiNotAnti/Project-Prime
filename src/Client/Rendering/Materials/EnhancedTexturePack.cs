            ...
            List<EnhancementPackIssue> issues, EnhancementResourceBudget budget)
        {
            ...
            if (!TryNormalizePngPath(relativePath, out string normalized))
            {
                issues.Add(new EnhancementPackIssue(EnhancementPackIssueKind.UnsafeTexturePath,
                    "Texture path must be a contained relative PNG path.", key, relativePath));
                return null;
            }
            ...
            var asset = new EnhancedTextureAsset(normalized, width, height, encoded, rgba8);
