...
namespace MphRead
{
    public static class Selection
    {
        ...
        public static bool OnKeyDown(KeyboardKeyEventArgs e, Scene scene)
        {
            ...
            if (e.Key == Keys.X && ScenePresentation.Get(scene).AllowCameraMovement && ScenePresentation.Get(scene).CameraMode != CameraMode.Player)
            {
        ...
        private static bool FilterEntity(EntityBase entity, Scene scene)
        {
            return entity.Type == EntityType.BeamEffect || entity.Type == EntityType.BeamProjectile ||
                entity.GetModels().Any(m => (ScenePresentation.Get(scene).ShowAllEntities || m.Active)
                    && (ScenePresentation.Get(scene).ShowAllEntities || ScenePresentation.Get(scene).ShowInvisibleEntities || !m.IsPlaceholder));
        }
        ...
        private static void LookAtSelection(Scene scene, bool control, bool shift)
        {
            if (control)
            {
                ...
                if (target != null)
                {
                    ScenePresentation.Get(scene).LookAt(target.Position);
                }
            }
            else if (Node != null) // node or mesh
            {
                ScenePresentation.Get(scene).LookAt(Node.Animation.Row3.Xyz);
            }
            else if (Entity != null) // entity or model instance
            {
                ScenePresentation.Get(scene).LookAt(Entity.Position);
            }
        }
    }
}
