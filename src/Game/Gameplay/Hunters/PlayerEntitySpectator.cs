    ...
    public partial class PlayerEntity
    {
        ...
        internal void ServerSetSpectating(bool spectating)
        {
            ...
            if (spectating)
            {
                ...
                foreach (NodeDefenseEntity node in _scene.GetNodeDefenseEntities()) { node.ReleaseServerPlayer(this); }
                foreach (OctolithFlagEntity flag in _scene.GetOctolithFlagEntities()) { flag.ReleaseServerPlayer(this); }
                if (_scene.Match.PrimeHunter == SlotIndex) { _scene.Match.PrimeHunter = -1; }
