...
namespace MphRead
{
    ...
    public partial class Scene : IDisposable
    {
        ...
        public int GetInitialEntityState(int entityId, bool active,
            int activeState = 3, int inactiveState = 1)
        {
            if (entityId == -1 || RoomId < 27) { return 0; }
            if (RoomId > 92 || entityId > 239) { return 1; }
            return ((active ? activeState : inactiveState) & 3) - 1;
        }
