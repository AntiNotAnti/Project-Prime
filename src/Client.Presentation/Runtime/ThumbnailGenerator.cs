...
namespace MphRead.Mods
{
    ...
    public static partial class ThumbnailGenerator
    {
        ...
        private static bool HasPlayerSpawn(string roomKey, RoomMetadata meta)
        {
            ...
            // Custom map binaries live in the content-addressed cache and are
            // exposed through a selected-map overlay only while that room is
            // being prepared. Probing the immutable base-game entity directory
            // here therefore reports a missing file for every valid custom map.
            // The map compiler already validates custom player spawns; retain
            // this legacy entity-file probe only for built-in rooms.
            if (!ShouldProbeBaseGameEntityFile(meta.Id))
            {
                _spawnCache[roomKey] = true;
                return true;
            }
            bool found = false;
            try
        ...
        internal static bool ShouldProbeBaseGameEntityFile(int roomId)
            => roomId < RuntimeRoomRegistry.FirstCustomRoomId;
