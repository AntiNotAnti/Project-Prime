    ...
    public partial class PlayerPresentation
    {
        ...
        public static void ProcessInput(Scene scene, KeyboardState keyboardState, MouseState mouseState, bool noPlayerInput)
        {
            ...
            for (int i = 0; i < scene.Players.Count; i++)
            {
                ...
                presentation.ProcessBottomScreenInput(
                    noPlayerInput || Mods.SpectatorMode.IsSpectating);
                if (presentation.BottomScreenSuppressesMouseThisStep)
                {
                    presentation.SuppressBottomScreenGameplayMouse();
                    ResetMorphBallBoostDetectors();
                }
                else
                {
                ...
                ApplyBottomScreenActions(player);

                if (!presentation.BottomScreenSuppressesMouseThisStep
                    && mouseSnap.IsButtonDown(MouseButton.Left)
                    && prevMouseSnap?.IsButtonDown(MouseButton.Left) != true)
                {
        ...
        private static void ApplyBottomScreenActions(PlayerEntity player)
        {
            BottomScreenAction actions = player.GetPresentation().TakeBottomScreenAction();
            if ((actions & BottomScreenAction.NextWeapon) != 0)
            {
                ApplyStylusButton(player.GetPresentation().Bindings.NextWeapon,
                    down: true, pressed: true);
            }
            if ((actions & BottomScreenAction.Morph) != 0)
            {
                ApplyStylusButton(player.GetPresentation().Bindings.Morph,
                    down: true, pressed: true);
            }
            if (actions != BottomScreenAction.None) player.ModNoteInput();
        }
