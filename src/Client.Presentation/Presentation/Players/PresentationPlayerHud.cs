...
namespace MphRead.Entities
{
    public partial class PlayerPresentation
    {
        ...
        internal static Vector2 ResolveReticlePosition(Vector2 current,
            float w, Vector2 projected)
        {
            if (TryNormalizeReticlePosition(w, projected, out Vector2 normalized))
            {
                return normalized;
            }
            return float.IsFinite(current.X) && float.IsFinite(current.Y)
                ? current
                : ReticleCenter;
        }
        ...
        public void DrawHudObjects()
        {
            ...
            if (CurrentRadarProfile.Style == Hud.Radar.RadarStyle.Enhanced
        ...
        public void DrawAmmoBar()
        {
            ...
            if (radarProfile.Style == Hud.Radar.RadarStyle.Enhanced
                && radarProfile.Anchor == Hud.Radar.RadarAnchor.TopRight)
            {
