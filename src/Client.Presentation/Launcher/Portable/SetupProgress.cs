...
namespace MphRead.Mods.Launcher
{
    ...
    /// <summary>
    /// Coalesces an extractor's bursty, potentially concurrent output into at
    /// most one pending presentation update. Parsing stays off the UI thread;
    /// the scheduled callback only paints the latest complete state.
    /// </summary>
    internal sealed class SetupProgressUpdateQueue
    {
        private readonly object _gate = new();
        private readonly Action<Action> _schedule;
        private readonly Action<string, double, string> _present;
        private readonly SetupProgress _progress = new();
        private string _status;
        private bool _updateQueued;
        private bool _finished;

        public SetupProgressUpdateQueue(string initialStatus, Action<Action> schedule,
            Action<string, double, string> present)
        {
        ...
        public void Report(string line)
        {
            ...
            lock (_gate)
            {
                ...
                _status = Tail(_status, line);
                _progress.Observe(line);
                schedule = !_updateQueued;
                _updateQueued = true;
            }
            if (schedule)
            {
        ...
        public void Finish(bool ok, string finalLine)
        {
            bool schedule;
            lock (_gate)
            {
                ...
                _finished = true;
                _status = Tail(_status, finalLine);
                _progress.Finish(ok);
                schedule = !_updateQueued;
                _updateQueued = true;
            }
        ...
        private void Flush()
        {
            ...
            _present(status, fraction, stage);
        }
