...
namespace MphRead.Mods.Launcher.Gui
{
    ...
    internal sealed class HomeView : UserControl, IAsyncDisposable, IDisposable
    {
            ...
            IMatchTransitionMenuActions? transitionActions = null)
        {
            ...
            view.HunterChangeRequested += hunter =>
                _ = ExecuteTransitionActionAsync(actions,
                    cancellation => actions.RequestHunterChangeAsync(hunter, cancellation));
        ...
        private async Task ChooseRom(MenuEntry button, Note log)
        {
            ...
            var progress = new SetupProgressUpdateQueue(log.Text,
                action => Dispatcher.UIThread.Post(action),
                (text, fraction, stage) =>
                {
                    log.Text = text;
                    _setupProgress.Set(fraction, stage);
                });
            ...
            // install needs.
            bool ok = await Task.Run(() => GameFiles.RunSetup(path, progress.Report));
            if (scratch != null)
            {
                try
                {
            ...
            if (ok)
            {
                await RenderPreviews(log, progress.Report);
            }
            progress.Finish(ok, ok ? "Ready to play." : "Setup did not finish.");
            button.IsEnabled = true;
            button.Title = "Choose your .nds file";
        ...
        /// with the program.
        /// </summary>
        private async Task RenderPreviews(Note log, Action<string>? progress = null)
        {
            ...
            if (progress != null)
            {
                progress("Rendering map previews...");
                await ThumbnailHost.RenderMissingAsync(progress);
            }
            else
            {
        ...
        /// </summary>
        private async Task OpenNodeBrowserAsync(bool createLobby)
        {
        ...
        private void OpenHost() => _ = ObserveUiTask(OpenNodeBrowserAsync(createLobby: true), "open host");

        private void OpenJoin() => _ = ObserveUiTask(OpenNodeBrowserAsync(createLobby: false), "open join");
        ...
        private static async Task ObserveUiTask(Task operation, string description)
        {
            ...
            {
                Console.WriteLine($"[launcher] {description} failed: {ex.Message}");
            }
        }
