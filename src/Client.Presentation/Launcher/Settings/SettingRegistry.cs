...
public static class SettingRegistry
{
    ...
    private static SettingDescriptor Launcher(string id, string label,
        ...
        SettingPlatform platform = SettingPlatform.All, bool advanced = false,
        bool requiresRestart = false, string? description = null)
        => new()
        {
            Id = id, Label = label, Description = description, Category = category,
            Scope = scope, Kind = kind, Advanced = advanced,
            RequiresRestart = requiresRestart, Platform = platform, RowId = rowId,
            PersistenceFile = LauncherFile,
    ...
    private static IReadOnlyList<SettingDescriptor> BuildDescriptors()
    {
        var descriptors = new List<SettingDescriptor>
        {
            ...
            Launcher("graphics.api", "Graphics API", SettingCategory.Graphics,
                SettingControlKind.Choice, "graphics_api", SettingRowIds.GraphicsApi,
                choices: new[]
                {
                    new SettingChoice("auto", "Automatic"),
                    new SettingChoice("d3d12", "Direct3D 12"),
                    new SettingChoice("vulkan", "Vulkan")
                }, defaultValue: "auto", scope: SettingScope.PlatformPreference,
                platform: SettingPlatform.Desktop, advanced: true,
                requiresRestart: true,
                description: "The persistent SDL host selects the GPU device at process start; restart Project Prime after changing this value."),
            ...
            Json("hud.pro-size", "Pro HUD size", SettingCategory.Hud, SettingControlKind.Choice,
                "ProHudSize", SettingRowIds.ProHudSize, dependsOn: "hud.pro",
                choices: ChoiceList("Compact", "Standard", "Large")),
            Json("hud.pro-safe-area", "Pro HUD safe area", SettingCategory.Hud,
                SettingControlKind.Toggle, "ProHudSafeArea", SettingRowIds.ProHudSafeArea,
                dependsOn: "hud.pro"),
            Json("hud.pro-high-contrast", "Pro HUD high contrast", SettingCategory.Hud,
                SettingControlKind.Toggle, "ProHudHighContrast",
                SettingRowIds.ProHudHighContrast, dependsOn: "hud.pro"),
            Json("hud.font", "HUD font", SettingCategory.Hud, SettingControlKind.Choice,
                "HudFont", SettingRowIds.HudFont,
                choices: new[]
                {
                    new SettingChoice("original", "Original"),
                    new SettingChoice("modern", "Modern")
                }, defaultValue: "modern",
                description: "Modern uses a clean proportional face; Original preserves the game font."),
            ...
            Json("hud.crosshair-style", "Crosshair type", SettingCategory.Hud,
                ...
                dependsOn: "hud.pro", choices: ChoiceList("Cross", "Dot", "Cross + dot",
                    "Circle", "Brackets", "Ring + dot", "T + dot", "Box",
                    "Precision", "Shotgun")),
            ...
            Json("hud.hit-marker-size", "Hit marker size", SettingCategory.Hud,
                SettingControlKind.Slider, "HitMarkerSize", SettingRowIds.HitMarkerSize,
                minimum: .5, maximum: 2, step: .05, defaultValue: "1.0"),
            Json("hud.hit-marker-opacity", "Hit marker opacity", SettingCategory.Hud,
                SettingControlKind.Slider, "HitMarkerOpacity", SettingRowIds.HitMarkerOpacity,
                minimum: .2, maximum: 1, step: .05, defaultValue: "1.0"),
            Json("hud.hit-marker-palette", "Hit marker palette", SettingCategory.Hud,
                SettingControlKind.Choice, "HitMarkerPalette", SettingRowIds.HitMarkerPalette,
                choices: new[]
                {
                    new SettingChoice("Classic", "Classic"),
                    new SettingChoice("HighContrast", "High contrast"),
                    new SettingChoice("Colorblind", "Colorblind"),
                    new SettingChoice("Monochrome", "Monochrome")
                }),
            Json("hud.hit-marker-animation", "Hit marker animation", SettingCategory.Hud,
                SettingControlKind.Slider, "HitMarkerAnimation", SettingRowIds.HitMarkerAnimation,
                minimum: 0, maximum: 1, step: .05, defaultValue: "1.0"),
            Json("hud.headshot-cue", "Headshot cue", SettingCategory.Hud,
                SettingControlKind.Toggle, "HeadshotCue", SettingRowIds.HeadshotCue),
            Json("hud.headshot-kill-sound", "Headshot kill sound", SettingCategory.Hud,
                SettingControlKind.Choice, "HeadshotKillSound",
                SettingRowIds.HeadshotKillSound, dependsOn: "hud.headshot-cue",
                choices: ChoiceList("Warzone", "Boom", "Announcer", "Juicy", "Cartoon"),
                defaultValue: "Warzone",
                description: "Selects the local sound played after an authoritative headshot kill."),
            ...
            Controls("controls.controller-anti-deadzone", "Look anti-deadzone", SettingControlKind.Slider,
                "gamepad_look_anti_deadzone", SettingRowIds.ControllerAntiDeadzone,
                group: "Stick response", minimum: 0, maximum: .5, step: .01,
                aliases: new[] { "gamepad_anti_deadzone" }),
            Controls("controls.controller-look-smoothing", "Adaptive look smoothing", SettingControlKind.Slider,
                "gamepad_look_smoothing_seconds", SettingRowIds.ControllerLookSmoothing,
                group: "Look response", minimum: 0, maximum: .25, step: .001,
                aliases: new[] { "gamepad_look_smoothing" }),
            ...
            Info("system.export-settings", "Export all settings", SettingCategory.System,
                SettingControlKind.Action, SettingRowIds.ExportSettings),
            Info("system.import-settings", "Import settings", SettingCategory.System,
                SettingControlKind.Action, SettingRowIds.ImportSettings),
            ...
            Controls("controls.stylus-bottom-screen-aim-mode", "Touch-screen aiming", SettingControlKind.Choice,
                "bottom_screen_aim_mode", SettingRowIds.BottomScreenAimMode, group: "General",
                choices: ChoiceList("Free tablet", "True DS surface")),
