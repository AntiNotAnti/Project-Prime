...
namespace MphRead
{
    ...
    public partial class ScenePresentation : IScenePresentation
    {
        ...
        public ScenePresentation(Scene world, Vector2i size, KeyboardState keyboardState, MouseState mouseState,
            Action<string> setTitle, Action close, FrameTiming timing,
            ISceneServices? sceneServices = null,
        ...
        public void OnLoad()
        {
            ...
            // Passive replay/killcam perspectives can select a player that was
            // not Main while InitializeWorld built presentation state. The HUD
            // is still consumed by simulation and drawing for that perspective,
            // so initialize it once at the scene ownership boundary.
            if (World.LocalPlayer is { } localPlayer
                && localPlayer.LoadFlags.TestFlag(LoadFlags.SlotActive))
            {
        ...
        private void OnSimulationStep()
        {
            ...
            if (ProcessFrame)
            {
                ...
                if (!PlaybackSeeking)
                {
                    ...
                    bool suppressBottomScreenLook = World.LocalPlayer!
                        .GetPresentation().PrepareBottomScreenActivation(
                            _keyboardState, _mouseState, noPlayerInput);
                    bool allowLocalLook = CanCaptureSimulationLook && !noPlayerInput
                        && !suppressBottomScreenLook;
                    Mods.Input.GamepadInput.BeginFrame(allowLocalLook,
                        World.LocalPlayer?.EquipInfo.Zoomed == true, Timing.Discontinuities);
        ...
        public void OnDrawFrame()
        {
            ...
            float capturedRenderFraction = Timing.RenderAlpha;
            ...
            {
                Mods.Input.GamepadInput.SampleNativeFrame(Timing.Discontinuities);
            }
            else
            {
            ...
            Hud.Radar.RadarDeviceClass radarDevice =
#if ANDROID
                Hud.Radar.RadarDeviceClass.Handheld;
#else
                Hud.Radar.RadarDeviceClass.Desktop;
#endif
            if (Hud.Radar.RadarSettings.ForContext(
                    World.Match.Rules.Mode.ToString(), radarDevice).Style
                == Hud.Radar.RadarStyle.Enhanced)
            ...
            _renderFrame.CaptureState(
                ...
                new RenderFrameOptions(
                    ...
                    false,
                    frameQuality,
                    Mods.RenderOptions.VisualStyle,
                    Mods.RenderOptions.TexturePackEnabled),
            ...
            _glesWorldContext.BeginFrame(_shaderLocations,
                ...
                Mods.RenderOptions.TexturePackEnabled ? _glesEnhanced : null);
            for (int i = 0; i < _renderFrame.Submissions.Count; i++)
            {
                DrawSubmission submission = _renderFrame.Submissions[i];
            ...
            uint presentationTick, float renderFraction)
        {
            ...
            bool celEnabled = Mods.RenderOptions.VisualStyle == Mods.VisualStyle.Cel
        ...
        {
            if (_fpsCounter.Record(Stopwatch.GetTimestamp(), out float framesPerSecond))
            {
        ...
        private void UpdateUniforms()
        {
            ...
            GL.Uniform1(_shaderLocations.CelBands,
                SdlGpuCelSurface.ShaderStyleCode(_renderFrame.Options));
        ...
        public void OnKeyDown(WindowKeyEvent e)
        {
            ...
            else if (e.Key == Keys.D5 && e.Shift)
            {
                ...
                {
#if ANDROID
                    ScreenCapture.Screenshot(Size.X, Size.Y, Timing.TotalFrames);
#else
