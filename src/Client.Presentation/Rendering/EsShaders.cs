...
// Visual style: 0 original, positive cel bands, -1 flat, -2 pixelated,
// -3 retro. Only the scene is drawn through this program;
...
vec2 styled_texture_uv(vec2 uv)
{
    ...
    vec2 dimensions = vec2(max(textureSize(tex, 0), ivec2(1)));
...
vec3 retro_color(vec3 color_value)
{
    float dither = ((bayer4(gl_FragCoord.xy) + 0.5) / 16.0 - 0.5) / 31.0;
    return floor(clamp(color_value + dither, 0.0, 1.0) * 31.0 + 0.5) / 31.0;
}
...
void main()
{
    ...
    if (use_texture) {
        ...
        vec4 texcolor = use_pal_override ? vec4(pal_override_color.xyz, texture(tex, texture_uv).w) : texture(tex, texture_uv);
