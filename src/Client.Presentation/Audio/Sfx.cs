...
namespace MphRead.Sound
{
    public static class Sfx
    {
        ...
        internal static void StopPresentationCore()
        {
            ...
            // sequence. MusicPlayer.Remove cancels and fences any decoder
            // still warming up, while retaining the process output device.
            Music.Stop();
            // A scene may have queued a sequence whose decoder is still
            // warming up. Cancellation is nonblocking; a stale completion is
            // disposed by the coordinator and cannot attach afterwards.
        ...
        private static void HandleRequestCore(AudioRequest request)
        {
            ...
            switch (request.Kind)
            {
                case AudioRequestKind.UpdateSource:
                    if (source == null) { break; }
                    if (request.Id == -1)
                    {
                        source.Position = Instance.GetListenerPosition();
                        source.ReferenceDistance = source.MaxDistance = Single.MaxValue;
                    }
                    else if (request.Id >= 0 && request.Id < Instance.RangeData.Count)
                    {
                        Sound3dEntry range = Instance.RangeData[request.Id];
                        source.ReferenceDistance = range.FalloffDistance / 4096f;
                        source.MaxDistance = range.MaxDistance / 4096f;
                    }
                    break;
                case AudioRequestKind.Play:
                    if (request.Id < 0) { break; }
                    if ((request.Id & 0x8000) != 0)
                    {
                        Instance.PlayDgn(request.Id, source, request.Loop, request.NoUpdate,
                            request.Recency, request.Cancellable, request.AmountA, request.AmountB);
                    }
                    else if ((request.Id & 0x4000) != 0)
                    {
                        Instance.PlayScript(request.Id, source, request.NoUpdate,
                            request.Recency, request.SourceOnly, request.Cancellable);
                    }
                    else
                    {
                        Instance.PlaySample(request.Id, source, request.Loop, request.NoUpdate,
                            request.Recency, request.SourceOnly, request.Cancellable);
                    }
                    break;
                case AudioRequestKind.PlayFree: Instance.PlayFreeSfx(request.Id); break;
                case AudioRequestKind.PlayEnvironment:
                    if (source != null) { Instance.PlayEnvironmentSfx(request.Id, source); }
                    break;
                case AudioRequestKind.StopSource:
                    if (source != null) { Instance.StopSoundFromSource(source, request.Force); }
                    break;
                case AudioRequestKind.StopSourceSound:
                    if (source != null) { Instance.StopSoundFromSource(source, request.Id); }
                    break;
                case AudioRequestKind.StopFreeSound: Instance.StopSoundById(request.Id); break;
                case AudioRequestKind.StopFreeScripts: Instance.StopFreeSfxScripts(); break;
                case AudioRequestKind.PauseFreeScripts: Instance.SetPausedFreeSfxScripts(request.Force); break;
                case AudioRequestKind.QueueStream: Instance.QueueStream(request.Id, request.Delay, request.Expiration); break;
                case AudioRequestKind.PlayRoomMusic: Music.PlayRoomMusic(request.Id, request.Track); break;
                case AudioRequestKind.PlayMusic: Music.PlayMusic((MusicId)request.Id); break;
                case AudioRequestKind.PlayMusicSequence: Music.PlaySeq((SeqId)request.Id); break;
                case AudioRequestKind.UpdateMusicTempo: Music.UpdateTempo((ushort)request.Id, request.Delay); break;
                case AudioRequestKind.PauseMusic: Music.Pause(); break;
                case AudioRequestKind.ResumeMusic: Music.PlayPausedMusic(); break;
                case AudioRequestKind.ResetSoundMutes:
                    SfxMute = false; ForceFieldSfxMute = TimedSfxMute = LongSfxMute = 0;
                    break;
                case AudioRequestKind.ChangeForceFieldMute:
    ...
    public class SfxInstance : SfxInstanceBase
    {
        ...
        private void UpdateDgn(SoundInstance inst, float amountA, float amountB)
        {
            ...
            for (int i = 0; i < inst.Count; i++)
            {
                ...
                inst.Pitch[i] = SoundPitch.CalculatePitchDiv(pitchFac);
            }
        }
        ...
        private class EnvironmentItem
        {
            ...
            {
                SfxId = (int)sfxId;
                Source = new SoundSource(new AudioRequests());
            }
