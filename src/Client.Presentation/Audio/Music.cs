...
namespace MphRead
{
    public static class Music
    {
        ...
        public static void Init(ClientPresentationContentState? content = null)
        {
            ...
            // The process-owned SoundFlow device is initialized by
            // MusicPlayer's lifetime owner. Do not decode a sequence here:
            // scene construction must not synchronously wait for music.
        }
            ...
            ushort fadeOutFrames = 0, ushort fadeInFrames = 0)
        {
            ...
            if (!queue)
            {
                _nextTrackNotReady = false;
                Stop();
                // Loading is now asynchronous for both legacy readiness
                // modes. UpdateMusic observes completion and starts playback
                // on the presentation owner thread.
                _isReady = false;
        ...
        public static void UpdateMusic()
        {
            ...
            if (!_isReady && !MusicPlayer.Loading)
            {
                ...
                {
                    // A replay restore can request a paused sequence before
                    // its decoder has completed. Keep the restored state
                    // paused when the graph is finally published.
                    MusicPlayer.Pause();
                }
            }
    ...
    public static class MusicPlayer
    {
        ...
        private static readonly MusicLoadCoordinator _loadCoordinator =
            new(DecodeAsync, PublishLoaded);
        private static bool _shutdownRequested;
        private static bool _playWhenReady;
        private static float _playWhenReadyVolume;
        private static bool _pauseWhenReady;
        ...
        public static bool Loading => _loadCoordinator.IsBusy;

        internal static MusicLoadCoordinator.Diagnostics LoadDiagnostics
            => _loadCoordinator.Snapshot;

        public static void Load(SeqId seqId, ushort tracks = UInt16.MaxValue, float volume = 1)
            => _ = LoadAsync(seqId, tracks, volume);

        /// <summary>
        /// Admits one latest-wins music load and returns its completion without
        /// waiting for decode or publication. Callers that need to sequence a
        /// presentation state transition may await the returned task; the
        /// compatibility <see cref="Load"/> wrapper observes it nonblocking.
        /// </summary>
        internal static Task<MusicLoadCoordinator.Completion> LoadAsync(
            SeqId seqId, ushort tracks = UInt16.MaxValue, float volume = 1)
        {
            if (seqId == SeqId.None)
            {
                Stop();
                return ImmediateLoadCompletion(seqId, tracks, volume,
                    MusicLoadCoordinator.CompletionStatus.Canceled);
            }
            ...
            lock (_lifecycleGate)
            {
                lock (_stateGate)
                {
                    ...
                    {
                        return ImmediateLoadCompletion(seqId, tracks, volume,
                            MusicLoadCoordinator.CompletionStatus.Shutdown);
                    }
                    if (!Available)
                    {
                        return ImmediateLoadCompletion(seqId, tracks, volume,
            ...
            return _loadCoordinator.Enqueue(seqId, tracks, volume,
                LogLoadCompletion).Completion;
        }

        private static Task<MusicLoadCoordinator.Completion> ImmediateLoadCompletion(
            SeqId seqId, ushort tracks, float volume,
            MusicLoadCoordinator.CompletionStatus status)
        {
            MusicLoadCoordinator.LoadRequest request =
                new(0, seqId, tracks, volume);
            return Task.FromResult(new MusicLoadCoordinator.Completion(
                request, status, TimeSpan.Zero, null));
        }

        private static void LogLoadCompletion(MusicLoadCoordinator.Completion result)
        {
            if (result.Status != MusicLoadCoordinator.CompletionStatus.Failed) return;
            string detail = result.Error?.Message ?? "unknown error";
            Console.WriteLine($"[sound] music load failed for "
                + $"sequence {result.Request.Sequence}: {detail}");
        }

        private static Task<IDisposable> DecodeAsync(
            MusicLoadCoordinator.LoadRequest request, CancellationToken cancellation)
            => Task.Run(() => Decode(request, cancellation), cancellation);

        private static IDisposable Decode(
            MusicLoadCoordinator.LoadRequest request, CancellationToken cancellation)
        {
            ...
            try
            {
                ...
                string path = Paths.Combine(Paths.FileSystem, "_seq",
                    Metadata.SequenceFiles[(int)request.Sequence]);
                ...
                stream = new NCSFPlayerStream(path, (uint)_sampleRate, Interpolation.None, skipSilenceOnStartSec: 5,
                    ...
                    playForever: true, request.Volume, channelMutes: 0, trackMutes: 0, ignoreVolume: false);
                ...
                return new LoadedMusic(stream, provider, player);
            }
            catch
            {
                DisposeUnattached(player, provider, stream);
                throw;
            }
        }

        private static bool PublishLoaded(
            MusicLoadCoordinator.LoadRequest request, IDisposable result)
        {
            _ = request;
            if (result is not LoadedMusic loaded) return false;
            if (loaded.Stream is not { } stream
                || loaded.Provider is not { } provider
                || loaded.Player is not { } player)
            {
                return false;
            }
            lock (_lifecycleGate)
            {
                lock (_stateGate)
                {
                    if (!Available || _shutdownRequested) return false;
                    ...
                    // Publication is the only point where the decoded graph
                    // enters the process-owned SoundFlow device.
                    _stream = stream;
                    _provider = provider;
                    _player = player;
                    loaded.Detach();
                    if (_playWhenReady)
                    {
                        stream.VolumeModification = Math.Clamp(_playWhenReadyVolume, 0, 1);
                        player.Play();
        ...
        {
            internal LoadedMusic(NCSFPlayerStream stream, RawDataProvider provider,
                SoundPlayer player)
            {
                Stream = stream;
                Provider = provider;
            ...
            internal NCSFPlayerStream? Stream { get; private set; }
            internal RawDataProvider? Provider { get; private set; }
            internal SoundPlayer? Player { get; private set; }
        ...
        public static void Play(float volume)
        {
            lock (_lifecycleGate)
            {
                lock (_stateGate)
                {
                    ...
                    {
                        _playWhenReady = true;
                        _playWhenReadyVolume = volume;
                        _pauseWhenReady = false;
                        return;
                    }
                    _playWhenReady = false;
                    _pauseWhenReady = false;
                    _stream!.VolumeModification = Math.Clamp(volume, 0, 1);
                    _player.Play();
                }
            }
        }
        ...
        /// <summary>
        /// Requests playback when the current asynchronous load publishes.
        /// This is the nonblocking replacement for the old sound-test
        /// Load/synchronous-wait/Play sequence.
        /// </summary>
        internal static void PlayWhenLoaded(float volume) => Play(volume);
        ...
        internal static void CancelLoad() => _loadCoordinator.Cancel();
        ...
        public static void Remove(bool shutdown = false)
        {
            ...
            // Final process teardown is the one explicit synchronous drain.
            // Normal scene release returned above without waiting for decode.
            _loadCoordinator.DrainAsync().GetAwaiter().GetResult();
            lock (_lifecycleGate)
            {
                lock (_stateGate)
                {
