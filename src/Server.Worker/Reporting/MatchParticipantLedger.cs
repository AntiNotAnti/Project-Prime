...
public sealed class MatchParticipantLedger
{
    ...
    private Guid _match;
    private readonly Guid? _assignedMatchId;
    ...
    public MatchParticipantLedger(Guid server, Guid incarnation, string build, Func<DateTimeOffset>? utc = null, Guid? matchId = null)
    { _server = server; _incarnation = incarnation; _build = build; _utc = utc ?? (() => DateTimeOffset.UtcNow);
      if (matchId == Guid.Empty) throw new ArgumentException("Match identity must be nonempty.", nameof(matchId));
      _assignedMatchId = matchId; }

    public void BeginPlaying(Scene scene, ServerNetwork network, uint tick)
    {
        if (Report != null) return;
        bool first = !Started;
        if (first) { Started = true; _match = _assignedMatchId ?? Guid.NewGuid(); _started = _utc().ToUniversalTime(); }
    ...
    public void LeaveSlot(Scene scene, int slot, uint tick, ParticipantExitReason reason = ParticipantExitReason.Disconnected, ulong connection = 0)
    {
        ...
        int last = participant.Spans.Count - 1;
        // A removal can run after this tick was already simulated. Report spans use an
        // exclusive end, so preserve the last observed played step just as Complete does.
        uint leftTick = participant.HasPlayed && Sequence32.IsNewer(participant.LastPlayedExclusive, tick)
            ? participant.LastPlayedExclusive : tick;
        participant.Spans[last] = participant.Spans[last] with { LeftTick = leftTick, ExitReason = reason };
    }
    ...
    private static MatchReportMetrics Snapshot(Scene scene, int slot, string name)
        => Convert(new PlayerMatchResult(scene.Match, slot, scene.Players[slot], name));
