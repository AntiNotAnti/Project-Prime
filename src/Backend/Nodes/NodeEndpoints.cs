...
/// <summary>
/// Requests a short-lived Node admission for the currently authenticated
/// account.  The visibility flag is part of the signed Node identity rather
/// than a separately trusted client value after admission.  It defaults to
/// true so older clients retain the pre-presence behavior.
/// </summary>
public sealed record NodeAdmissionRequest(Guid NodeId, bool PublicPresence = true);

/// <summary>Anonymous admission has the same explicit visibility contract as
/// registered admission.  The guest display name remains only a label; the
/// signed guest identity is generated by the Backend.</summary>
...
public static class NodeEndpoints
{
    public static void MapNodes(this WebApplication app)
    {
        ...
        }).Bodyless().AllowAnonymous().RequireRateLimiting(BackendRoutePolicy.Api);
        ...
        }).Bodyless().RequireRateLimiting(BackendRoutePolicy.MachinePreAuth);
            ...
            ILoggerFactory loggerFactory) =>
        {
            ...
            return Results.Ok(issuer.IssueNodeAdmission(new PlayerId(user.Id), profile.DisplayName,
                node.NodeId, node.PublicControlUri, request.PublicPresence));
        ...
        app.MapPost("/v1/guest-node-admissions",
                ...
                ILoggerFactory loggerFactory) =>
            {
                ...
                return Results.Ok(issuer.IssueGuestNodeAdmission(guestId, name, node.NodeId,
                    node.PublicControlUri, request.PublicPresence));
