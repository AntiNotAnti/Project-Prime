...
namespace MphRead.Mods.Network
{
    ...
    public sealed class ServerProcess : IDisposable
    {
        ...
        private ServerProcess(ProcessStartInfo start, string rotationFile)
        {
            ...
            (string Host, int Port, string Name)? listing = null, CancellationToken cancel = default)
        {
            ...
            try
            {
                ...
                server = new ServerProcess(start, rotationFile);
        ...
        {
            string executable = Environment.ProcessPath ?? throw new IOException("Cannot locate this game's executable.");
            bool dotnet = Path.GetFileNameWithoutExtension(executable).Equals("dotnet", StringComparison.OrdinalIgnoreCase);
            // Referencing test/tools executables must launch the game assembly,
            // not their own entry point. Production apphosts use their exact path.
            if (!dotnet && Assembly.GetEntryAssembly() != typeof(ServerProcess).Assembly)
            {
            ...
            if (dotnet)
            {
                string assembly = Path.Combine(AppContext.BaseDirectory, typeof(ServerProcess).Assembly.GetName().Name + ".dll");
                if (!File.Exists(assembly)) { throw new IOException("Cannot locate the game assembly for dotnet hosting: " + assembly); }
