...
namespace MphRead.Mods.Network
{
    ...
    public static class ServerContent
    {
        ...
        // File reads stay unchanged outside an explicitly scoped bake. Keeping
        // the hook at the file boundary captures shared parser dependencies.
        ...
        internal static void RecordModel(string path) => _recordModel?.Invoke(Path.GetFullPath(path));
        ...
        public static void Open(string directory, string version)
        {
            ...
            if (File.Exists(Path.Combine(path, ServerContentPack.ManifestName)))
            {
                _manifest = ServerContentPack.Validate(path, version);
                ClearContentCaches();
