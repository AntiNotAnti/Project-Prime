...
namespace MphRead.Mods
{
    ...
    public static class RenderOptions
    {
        ...
        public static int Scaled(int pixels)
            => Scaled(pixels, _resolutionScale);

        /// <summary>Apply an explicit effective scale without mutating user settings.</summary>
        public static int Scaled(int pixels, int scalePercent)
        {
            scalePercent = Math.Clamp(scalePercent, MinScale, 100);
            if (scalePercent >= 100)
            {
                return pixels;
            }
            return Math.Max(1, pixels * scalePercent / 100);
        }
