...
class ServerBundleContractTests(unittest.TestCase):
    ...
    def test_packager_keeps_worker_below_worker_directory_and_renames_only_node_apphost(self):
        ...
        self.assertIn("--map-artifacts", script)
    ...
    def test_custom_map_bundles_are_published_to_client_and_server(self):
        ...
        self.assertIn("CopyToPublishDirectory>Never", client)
        self.assertNotIn("CookCustomMapBundles", client)
        self.assertIn("tools/cook-maps.sh", server)
        self.assertIn('find "$MAP_ARTIFACTS" -maxdepth 1 -type f -name \'*.fpmap\'', server)
        self.assertNotIn('-mapdir "$ROOT/maps" -mapbundle all', server)
