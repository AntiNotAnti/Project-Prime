...
`rebooty.xyz:27889` is the configured default (`NetMasterConfig`) — a
...
`master_host=rebooty.xyz` in `launcher.txt`, server via
...
rebooty.xyz` on the directory) survive a redeploy and aren't in the
templates.
...
- **`-public` on the directory.** Right for a server behind a router, exactly
  ...
  player to their own machine. `-public rebooty.xyz` tells the
