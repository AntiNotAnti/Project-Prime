# Custom maps: low-level packing and importer evidence

> Current operational architecture is documented in `maps/README.md`. This file
> preserves the format discoveries and importer rationale behind the retained
> packers. References below to permissive bundles, generated AMHE1 files, and old
> one-shot flags describe the legacy implementation; new paths use manifest v2,
> `MapCatalog`, `MapCompiler`, the content-addressed cache, and runtime overlays.
...
- **Cooking bakes the texture pack if it is not already there**, and refuses
  ...
  level's own art, so it is gitignored like the room binaries and the compiler
  bakes it on demand or while cooking -- which means the machine that
...
Room binaries generated on a player's machine land in
`map-cache/<build-fingerprint>/`; they never land in extracted base content.
Texture baking is likewise a compiler dependency. A source project may refer to
a creator-owned `.pk3`, while a distributable v2 package carries its declared,
trimmed geometry and texture data only when redistribution is authorized.
...
```bash
ProjectPrime -q3convert path/to/level.pk3 -map LEVELNAME -name ROOM [-noclip]
```
...
ProjectPrime -q3convert level.pk3 -map NAME -name ROOM   # a .pk3 to a map file
ProjectPrime -q3maps  path/to/pak.pk3      # what levels are in there
ProjectPrime -q3shaders path/to/pak.pk3 -map wrackdm17   # what it draws with
ProjectPrime -mapgen                       # generate every map in maps/
ProjectPrime -mapgen "LONGEST YARD"        # just one
ProjectPrime -mapmaterials "MP3 PROVING GROUND"   # what textures can be borrowed
ProjectPrime -maptest "LONGEST YARD" -players 8 -seconds 22
ProjectPrime -thumbnail "LONGEST YARD"     # the launcher's picture
```

New code uses the per-map `MapCompiler`, dependency hashes, and the
content-addressed cache. The launcher compiles only a selected/required map; a
Worker validates and prepares maps independently and advertises only maps that
reach `Ready`. Failed maps remain diagnosable without making the entire client or
Node unavailable. Generated room binaries no longer modify AMHE1.

The legacy command path still routes through `ModEntry.TryHandle` and
`MapPreparation` compatibility adapters. Do not add new startup behavior there;
new callers should use `IMapCatalog`, `MapCompiler`, and the selected-map content
mount.
...
Legacy `maps/**/*.json` recipes remain discoverable and are adapted into the
new source model. Generated `.bin` files now land under
`map-cache/<build-fingerprint>/` and are exposed through a selected-map content
overlay; they are never copied into the player's extracted base content.
**The editable project and redistributable source are the things that belong in
git.** Generated binaries may incorporate data derived from a player's cartridge
dump and, for imported maps, another game, so the asset guard rejects them.

## On Android

Android installs declared data-only `.fpmap` assets atomically into the selected
Project Prime data root and consumes them through the same catalog/compiler/cache
contracts as desktop. It does not expose the desktop editor. Source-generated JSON
contracts are the trimming boundary, and package/generated content is never
unpacked into AMHE1. `CustomRooms.MapDirectory` remains configurable because APK
assets themselves are not enumerable filesystem files.

## Deliberately deferred surfaces

Community publishing/search services and advanced dynamic entities (doors, moving
platforms, force fields, teleporters, triggers, and a bounded event graph) remain
future phases. Their authoring types are reserved, but the compiler rejects them
until a verified deterministic runtime mapping exists. The current online path
already uses exact map package identity and trusted-origin acquisition; it does not
silently accept near-matching content.
