...
def main():
    ...
    for name in ('ProjectPrime.dll', 'ProjectPrime.runtimeconfig.json', 'ProjectPrime.deps.json'):
    ...
    with tempfile.TemporaryDirectory(prefix='parallax-movement-') as temporary:
        ...
        references = ''.join(
            ...
            for name in ('ProjectPrime', 'OpenTK.Mathematics'))
        ...
        subprocess.run([args.dotnet, 'exec', '--runtimeconfig', str(game / 'ProjectPrime.runtimeconfig.json'),
                        '--depsfile', str(game / 'ProjectPrime.deps.json'), str(output / 'Check.dll'),
