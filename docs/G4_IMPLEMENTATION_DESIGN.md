...
`POST /v1/game-tickets` authorizes the account and target server, then issues a short-lived signed game ticket containing schema version, issuer, PlayerId, authoritative display identity, server ID, server startup incarnation, issued/not-before/expiry times, unique ticket ID, and client join nonce. Use established signing/validation components with a fixed algorithm allowlist, explicit issuer/audience/key ID, bounded clock skew and payload length. Backend holds signing private keys; game servers receive public verification keys only. Account tokens, passwords and ticket contents never enter logs, rosters, demos, or match reports.
...
| `POST /v1/game-tickets` | Signed-in eligible player; target-server scoped short-lived credential |
