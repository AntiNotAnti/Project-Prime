...
Result signatures use type `pp-match-result+jwt`, audience `urn:project-prime:match-result`, and purpose `match-result-v1`. They are durable assertions without ticket expiry, not admission credentials. A verifier must require that exact type/audience/purpose, issuer, permitted ES256 key, and payload hash. They cannot validate as server-bound game tickets. Retain verification public keys for all published historical exports when rotating signing keys; private old keys are unnecessary. Ranking remains `policyPending`/null until the separate rating policy is approved.
