...
The four group lengths are asset facts. Their v1 names are a conservative
Project Prime mapping (Idle, Charge, Beam, Fly), not binary-confirmed retail
semantics.
...
placeholder gun; biped Power Beam shots still use the normal authoritative aim
and projectile pipeline. The HUD likewise uses the documented coloured Samus
portrait fallback until dedicated Guardian HUD art is authored.
...
envelope; it has no unrestricted flight. Holding Alt Attack charges using
